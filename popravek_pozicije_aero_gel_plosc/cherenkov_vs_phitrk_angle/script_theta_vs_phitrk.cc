...
void script_theta_vs_phitrk(){
            ...
            if(recHitx.p < 6) continue;
                ...
                if(65 < sqrt(recHitx.x * recHitx.x + recHitx.y * recHitx.y) && 100 > sqrt(recHitx.x * recHitx.x + recHitx.y * recHitx.y)){
                    ...
                    for(auto &photon : *photonsx){
                        ...
                        h2x->Fill(photon.getPhiCerTrk(), photon.getThetaCer());
                    }
                }
    }
    h2x->Scale(1./ntrkx);
            ...
            if(recHits.p < 6) continue;
                ...
                if(65 < sqrt(recHits.x * recHits.x + recHits.y * recHits.y) && 100 > sqrt(recHits.x * recHits.x + recHits.y * recHits.y)){
                    ...
                    for(auto &photon : *photonss){
                        if(photon.getMirror() != 0) continue;
                        h2s->Fill(photon.getPhiCerTrk(), photon.getThetaCer());
                    }
                }
    }
    h2s->Scale(1./ntrks);
    ...
    mgr1->Add(grs, "lp");
    mgr1->Draw("a");
    mgr1->SetTitle("Mean ThetaCer; PhiTrk; ThetaCer");
    TLegend *leg1 = new TLegend(0.86,0.35,0.99,0.45);
    ...
    mgr2->Add(sigs, "lp");
    mgr2->Draw("a");
    mgr2->SetTitle("Sigma ThetaCer; PhiTrk; ThetaCer");
    TLegend *leg2 = new TLegend(0.86,0.35,0.99,0.45);
    ...
    mgr3->Add(igs, "lp");
    mgr3->Draw("a");
    mgr3->SetTitle("Gauss Integral; PhiTrk; Accu");
    TLegend *leg3 = new TLegend(0.86,0.35,0.99,0.45);
    ...
    mgr4->Add(pgs, "lp");
    mgr4->Draw("a");
    mgr4->SetTitle("Pol1 Integral; PhiTrk; Accu");
    TLegend *leg4 = new TLegend(0.86,0.35,0.99,0.45);
