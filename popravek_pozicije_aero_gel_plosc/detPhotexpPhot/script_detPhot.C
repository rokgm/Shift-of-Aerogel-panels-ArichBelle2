...
void script_detPhot(){
    ...
    long int nentriesx = (Int_t)10000;//trx->GetEntries();
    long int nentriess = (Int_t)10000;//trs->GetEntries();
    ...
    for(int i=0; i<nentriesx; i++){
        ...
        h2x->Fill(recHitx.x, recHitx.y, detPhotx);
        rx->Fill(sqrt(recHitx.x*recHitx.x + recHitx.y*recHitx.y), detPhotx);
    }
    
    for(int i=0; i<nentriess; i++){
        trs->GetEntry(i);
        if(recHits.p < 6) continue;
        h2s->Fill(recHits.x, recHits.y, detPhots);
        rs->Fill(sqrt(recHits.x*recHits.x + recHits.y*recHits.y), detPhots);
    }
    ...
    rs->SetLineColor(2);
    rs->SetTitle("detPhot; Radius; NumPhotons");
