...
void script_detPhotexpPhot(){
    ...
    for(int i=0; i<nentriesx; i++){
        ...
        h2x->Fill(recHitx.x, recHitx.y, detPhotx - expPhotx.mu);
        rx->Fill(sqrt(recHitx.x*recHitx.x + recHitx.y*recHitx.y), detPhotx - expPhotx.mu);
    }
    
    for(int i=0; i<nentriess; i++){
        trs->GetEntry(i);
        if(recHits.p < 6) continue;
        h2s->Fill(recHits.x, recHits.y, detPhots - expPhots.mu);
        rs->Fill(sqrt(recHits.x*recHits.x + recHits.y*recHits.y), detPhots - expPhots.mu);
    }
    ...
    rs->Draw("hist");
    rx->Draw("same hist");
    rs->GetYaxis()->SetRangeUser(-17,10);
    rs->SetLineColor(2);
    rx->SetTitle("detPhot - expPhot; Radius; NumPhotons");
